...
from apps.review_workbench.site_navigation import bi, e, page, company_tabs, report_groups, report_list
...
def render_brief(company, rows, catalog, bundle=None):
    ...
    body+='<style>'+Path(__file__).with_name('company_brief.css').read_text()+'</style><script>'+Path(__file__).with_name('company_brief.js').read_text()+'</script>'
