...
from apps.review_workbench.query_runs import collections, contained, e, amount, period
...
def render_dataset(root, registry, company, collection_id, record_id=None):
    ...
    body += '<style>' + Path(__file__).with_name('query_runs.css').read_text() + '\n' + Path(__file__).with_suffix('.css').read_text() + '</style><div class="qr-workspace ds-workspace"><div class="qr-row"><h1>' + bi('数据库明细', 'Database records') + '</h1><a href="/companies/' + e(company['id']) + '/data/queries">' + bi('查看查询与 SQL', 'View queries & SQL') + '</a></div>'
    ...
    body += '<section class="ds-boundaries" id="data-updates"><h2>' + bi('后续数据如何更新', 'How new data is added') + '</h2><p>' + bi('当前是显式配置、生成新快照的方式；还没有自动增量同步。', 'Updates currently use explicit configuration and a new snapshot; automatic incremental sync is not implemented.') + '</p><ol>' + ''.join('<li>' + bi(*note) + '</li>' for note in collection['dataset']['update_notes']) + '</ol><p class="qr-muted">' + bi('旧查询继续绑定旧 snapshot_id。新快照不等于已审核或已采纳，也不会自动替换旧分析的输入。', 'Existing queries remain pinned to their snapshot_id. A new snapshot is not approval or adoption, and does not automatically replace old analysis inputs.') + '</p></section></div><script>' + Path(__file__).with_suffix('.js').read_text() + '</script>'
