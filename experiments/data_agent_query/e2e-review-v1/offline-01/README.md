...
每例 report.md、原文报告及 execution/session/event-* 保存可核查产物；Alphabet 例沿用既有显式任务计划。合成 run_question 例使用脚本 draft_plan，历史 API 的 origin=model 字段仅表示提案入口；本文件和 verification.json 明确标注实际来源为 scripted_fixture。
