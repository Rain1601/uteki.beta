...
def run(output):
    ...
    for expected in protocol["reference_numeric_rows"]:
        ...
        numeric_checks.append({"expected": expected, "found_count": len(matches),
                               ...
                               "matched": len(matches) == 1 and Decimal(matches[0]["value_decimal"]) == Decimal(expected["value_usd"]),
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.output.exists() and not (args.output/'manifest.json').exists() and not (args.output/'failure.json').exists():
