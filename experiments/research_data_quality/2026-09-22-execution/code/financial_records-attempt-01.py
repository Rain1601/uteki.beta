...
VERSION = "financial-records-v0.1"
CONCEPTS = {
...
def extract_financial_records(source_dir, index_dir, *, metrics=None, period_kinds=None):
    ...
    for tag in tree.iter():
        ...
        try:
            ...
            record["record_id"] = "obs-" + digest([manifest["source_snapshot_id"], logical_key(record), record["value_decimal"]])[:20]
...
def compute_ytd_difference(current, previous):
    ...
    if any(current[k] != previous[k] for k in ("entity_id", "metric", "unit", "dimensions", "accounting_basis", "value_kind")):
    ...
    record = {"record_type": "ComputedFact", "formula_id": "ytd_difference-v1",
              ...
              "value_decimal": format(Decimal(current["value_decimal"]) - Decimal(previous["value_decimal"]), "f"),
