...
def run(output):
    ...
    for name in ("annual", "q1", "q2"):
        ...
        result = extract_financial_records(ROOT / folder, ROOT / folder / index,
                                          mapping=ALPHABET_FINANCIAL_MAPPING, metrics=["revenue", "operating_income"] if name == "annual" else None)
        results[name] = result
    ...
    for step, expanded in (("step-03", False), ("step-04", True)):
        data = extract_transcript_records(ROOT / call_folder, ROOT / call_folder / call_index,
                                          company_id="alphabet", fiscal_calendar="calendar", expanded_guidance=expanded)
        results[step] = data
